...
-- TODO: parse tests, reorder tests
...
tests = testGroup "lsystems"
  ...
  , after AllSucceed "/lookupChar/" $
      testGroup "parse" (numberedTests parseTests)
  , after AllSucceed "/parse/" $
      testGroup "expandLSystem" (numberedTests expandLSystemTests)
...
expandTests =
  [ expand (rules arrowHead) (axiom arrowHead) 2 --> "N+M+N-M-N-M-N+M+N"
  , expand (rules dragon) (axiom dragon) 0       --> "MX"
  , expand (rules dragon) (axiom dragon) 1       --> "A+MX--MY+"
  , expand (rules dragon) (axiom dragon) 5
  ...
  , expand (rules tree) (axiom tree) 2
      --> "NN[-N[-M][+M][NM]][+N[-M][+M][NM]][NNN[-M][+M][NM]]"
  , expand (rules bush) (axiom bush) 3
...
parseTests = [ parse "NN[-N[-M][+M][NM]][+N[-M][+M][NM]][NNN[-M][+M][NM]]"
                 --> [ F, F
                     , B [R, F, B [R, F], B [L, F], B [F, F]]
                     , B [L, F, B [R, F], B [L, F], B [F, F]]
                     , B [F, F, F, B [R, F], B [L, F], B [F, F]]
                     ]
             , parse "N+M+N-M-N-M-N+M+N"
                 --> [F, L, F, L, F, R, F, R, F, R, F, R, F, L, F, L, F]
             , parse "A+MX--MY+"
                 --> [L, F, R, R, F, L]
             ]

expandLSystemTests :: [Assertion]
expandLSystemTests =
  [ expandLSystem tree 2
      --> [ F, F
          , B [R, F, B [R, F], B [L, F], B [F, F]]
          , B [L, F, B [R, F], B [L, F], B [F, F]]
          , B [F, F, F, B [R, F], B [L, F], B [F, F]]
          ]
  , expandLSystem arrowHead 2
      --> [F, L, F, L, F, R, F, R, F, R, F, R, F, L, F, L, F]
  , expandLSystem dragon 1
      --> [L, F, R, R, F, L]
  ]
